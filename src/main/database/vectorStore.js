...
class VectorStore {
    ...
    */
    async initialize(dbPath) {
        ...
        try {
            ...
            if (!this.manifestMatches(storedManifest)) {
                console.log("Embedding index outdated (model or chunking changed), rebuilding.");
            ...
            this.vectorStore = new LanceDB(this.buildEmbeddings(), { table: this.table });
            this.ready = true;
            return { rebuilt: false };
        } catch (err) {
            this.lastError = err;
    ...
    buildEmbeddings() {
        ...
        return {
            embedQuery: (query) => localEmbeddings.embedQuery(queryPrefix + query),
    ...
    async writeManifest(manifestPath) {
        fs.writeFileSync(manifestPath, `${JSON.stringify(indexManifest, null, 4)}\n`);
    }
    ...
    */
    async searchSimilarNotes(query, limit = maxResults, filter = null) {
        try {
            ...
            const queryEmbedding = await localEmbeddings.embedQuery(queryPrefix + query);
            ...
            const docs = rows.map((r) => {
                ...
                const pageContent = rawContent.replace(documentPrefixRegex, "");
                const metadata = {
                    ...(r.metadata ?? {}),
