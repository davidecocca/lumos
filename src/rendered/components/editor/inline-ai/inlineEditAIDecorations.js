...
const getOriginalTextMap = ({ doc, from, to }) => {
    ...
    const positionAtOffset = (offset) => {
        ...
        const segment = segments.find(({ textStart, textEnd }) => (
        ...
        const nextSegment = segments.find(({ textStart }) => offset < textStart)
...
const groupedDecorations = ({ doc, from, to, editedText }) => {
    ...
    const addInsertionWidget = (position, value) => {
        ...
        decorations.push(Decoration.widget(position, widget, {
            side: -1,
    ...
    changes.forEach((change) => {
        if (change.added) {
            addInsertionWidget(original.positionAtOffset(originalOffset), change.value)
            return
        }
        ...
        if (change.removed) {
            ...
            if (deleteFrom < deleteTo) {
                decorations.push(Decoration.inline(deleteFrom, deleteTo, {
...
export const setInlineEditAIPreview = (editor, payload) => {
    ...
    editor.view.dispatch(editor.state.tr.setMeta(inlineEditAIPluginKey, {
        type: 'setPreview',
        ...payload,
    }))
}
    ...
    addProseMirrorPlugins() {
        return [
            new Plugin({
                ...
                state: {
                    ...
                    apply: (transaction, decorationSet, oldState, newState) => {
                        ...
                        if (meta?.type === 'setPreview') {
                            return DecorationSet.create(newState.doc, groupedDecorations({
                                doc: newState.doc,
                                from: meta.from,
                                to: meta.to,
                ...
                props: {
                    decorations: (state) => inlineEditAIPluginKey.getState(state),
                },
            }),
        ]
    },
