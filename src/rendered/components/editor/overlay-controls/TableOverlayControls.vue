...
const getContainerElement = () =>
    props.containerRef?.value || props.containerRef || null;
...
const getSelectionNodeElement = () => {
    ...
    const domAtSelection = editorInstance.view.domAtPos(
        editorInstance.state.selection.from,
    );
    const domNode =
        domAtSelection.node?.nodeType === Node.TEXT_NODE
...
const getActiveCellElement = () =>
    getSelectionNodeElement()?.closest('td, th') || null;
const getActiveTableElement = () =>
    getActiveCellElement()?.closest('.tableWrapper, table') || null;
...
const getInteractionCellElement = () =>
    getHoveredCellElement() || getActiveCellElement();
const getInteractionTableElement = () =>
    getHoveredTableElement() || getActiveTableElement();
...
const setHoveredTableElements = (cellElement) => {
    ...
    hoveredTableElement.value =
        cellElement?.closest('.tableWrapper, table') || null;
    syncEditorState();
};
...
const handlePointerMove = (event) => {
    ...
    if (!nextCellElement) {
        if (
            targetElement &&
            getHoveredTableElement()?.contains(targetElement)
        ) {
            clearPendingHoverReset();
            return;
        }
...
const isHeaderRowActive = computed(() => {
    ...
    const tableElement =
        getInteractionTableElement()?.querySelector('table') ||
        getInteractionTableElement();
    const firstRowElement = tableElement?.querySelector('tr');
    const rowCells = firstRowElement ? [...firstRowElement.children] : [];

    return Boolean(
        rowCells.length && rowCells.every((cell) => cell.tagName === 'TH'),
    );
});
...
const isHeaderColumnActive = computed(() => {
    ...
    return Boolean(
        rows.length && rows.every((row) => row.children[0]?.tagName === 'TH'),
    );
});

const isControlDisabled = (control) =>
    !tableCapabilities.value[control.capability];

const getControlColor = (control) => {
    if (
        control.value === 'delete-column' ||
        control.value === 'delete-row' ||
        control.value === 'delete-table'
    ) {
        return 'error';
    }
...
const updateOverlayPosition = () => {
    ...
    if (!editorInstance || !containerElement || !shouldShowOverlay) {
        overlayState.value = {
            visible: false,
            columnTop: 0,
            columnLeft: 0,
    ...
    overlayState.value = {
        ...
        columnTop: clamp(
            tableRect.top -
                containerRect.top -
                COLUMN_TOOLBAR_TOP_GAP -
                COLUMN_TOOLBAR_HEIGHT,
        ...
        columnLeft: clamp(
            ...
            Math.max(
                COLUMN_TOOLBAR_WIDTH / 2,
                containerRect.width - COLUMN_TOOLBAR_WIDTH / 2,
            ),
        ),
        rowTop: clamp(
