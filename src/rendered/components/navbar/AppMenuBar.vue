<template>
    <div class="app-menu-bar no-drag d-flex align-center">
        ...
        <v-list density="compact" rounded="lg" min-width="320" class="pa-1 menu-list">
            <template v-for="(item, i) in visibleItems(menu.items)" :key="i">
                <v-divider v-if="item.type === 'separator'" class="my-1"></v-divider>
                <v-list-item
                v-else
                ...
                <div class="d-flex align-center ga-4">
                    ...
                    <span v-if="item.shortcut" class="menu-shortcut ml-auto flex-shrink-0">{{ item.shortcut }}</span>
                </div>
            </v-list-item>
        </template>
    </v-list>
...
const menuActions = {
    ...
    'toggle-chat': () => window.dispatchEvent(new Event(TOGGLE_NOTE_CHAT_EVENT)),
...
const menus = [
{
    ...
    { title: 'New Folder', shortcut: 'Ctrl+Shift+N', action: 'new-folder' },
    { type: 'separator' },
    { title: 'Save Current Note', shortcut: 'Ctrl+S', action: 'save-note', requiresNote: true },
    { type: 'separator' },
    { title: 'Close Window', shortcut: 'Ctrl+Q', action: 'close-window' },
    ],
},
{
    title: 'Edit',
    items: [
    { title: 'Undo', shortcut: 'Ctrl+Z', action: 'undo', requiresNote: true },
    { title: 'Redo', shortcut: 'Ctrl+Shift+Z', action: 'redo', requiresNote: true },
    { type: 'separator' },
    { title: 'Cut', shortcut: 'Ctrl+X', action: 'cut', requiresNote: true },
    { title: 'Copy', shortcut: 'Ctrl+C', action: 'copy', requiresNote: true },
    { title: 'Paste', shortcut: 'Ctrl+V', action: 'paste', requiresNote: true },
    { title: 'Select All', shortcut: 'Ctrl+A', action: 'select-all', requiresNote: true },
    { type: 'separator' },
...
},
{
    ...
    { title: 'Toggle Note Chat', shortcut: 'Ctrl+L', action: 'toggle-chat', requiresNote: true },
    { type: 'separator' },
    { title: 'Toggle Full Screen', shortcut: 'F11', action: 'toggle-fullscreen' },
    { type: 'separator' },
    { title: 'Toggle Developer Tools', shortcut: 'Ctrl+Shift+I', action: 'devtools' },
    ],
},
{
    title: 'Help',
    items: [
    { title: 'About Lumos', action: 'about' },
    ],
},
]
...
const visibleItems = (items) => {
    ...
    while (filtered.length && filtered[0].type === 'separator') filtered.shift()
    while (filtered.length && filtered[filtered.length - 1].type === 'separator') filtered.pop()
