    ...
    <v-card-text class="d-flex flex-column overflow-hidden h-100">
        ...
        <v-sheet class="mt-2 flex-grow-1 h-0 overflow-y-auto">
            ...
            <v-list v-else-if="visibleNotes.length === 0">
                <v-list-item
                :prepend-icon="hasSearchQuery ? 'ph-binoculars' : 'ph-clock-counter-clockwise'"
                class="text-medium-emphasis"
                >
                {{ hasSearchQuery ? 'No results' : 'No recent notes yet' }}
            </v-list-item>
        </v-list>
...
const isShowingSearchProgress = computed(() => isLoading.value || isSearchPending.value)
