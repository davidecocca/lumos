                ...
                <div class="model-trigger-wrap">
                    <v-menu location="top end">
                        ...
                        <v-list density="compact" rounded="lg" class="pl-1 pr-1 pt-2 pb-2">
                            <template v-for="group in groupedModelItems" :key="group.provider">
                            ...
                            >
                                ...
                                <template v-slot:append>
                                    <span v-if="isModelSelected(item.value) && item.supportedReasoningEfforts?.length" class="text-caption text-medium-emphasis me-1">
                                        {{ getReasoningLabel(item.value) }}
                                    </span>
                                    <v-menu v-if="item.supportedReasoningEfforts?.length" location="end">
                                        ...
                                        <v-list density="compact">
                                            <v-list-item
                                                ...
                                                @click.stop="setReasoningEffort(item.value, effort)"
                                            />
                                        </v-list>
                                    </v-menu>
                                </template>
...
const setReasoningEffort = (modelValue, effort) => {
    aiStore.setModelReasoningEffort('chat', modelValue.provider, modelValue.model, effort)
}

const getReasoningLabel = (modelValue) => formatReasoningEffort(
    aiStore.getModelReasoningEffort('chat', modelValue.provider, modelValue.model),
)
